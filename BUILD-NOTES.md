...
Largest table is 106k rows. Everything builds in seconds on an XS warehouse.

### Where the brief and the data disagreed

| Brief said | Data says | What changed |
|---|---|---|
| `loan` has 119 columns | 118 columns; `ctid` is not a real Postgres column, Fivetran synthesises it | Nothing material. The curated subset is 20 columns |
...
- **FS star has zero orphans** in every direction, verified. Join counts are
  exact: 2,948 relationships join 1:1 to performance metrics on all three keys,
  and 106,128 monthly rows join to relationships with no fanout and no loss.
- **`loan` PII is redundant, not just present.** `ssn`, `social_security_number`
  and `ssnumber` hold the identical value on all 39,717 rows; `ssnumber1` holds
  a different one; `dl` and `drivers_license` are identical on all rows. Six
...
- Instructor fallback schemas: `hicham_bab_consumer_packaged_goods`,
  `hicham_bab_energy`, `hicham_bab_financial_services`.
- Attendee schemas: `<firstname>_<lastname>_<schema>`, set by the Fivetran
  destination schema prefix.
- Each project exposes `source_database` and `source_schema` as vars in one
  block at the top of `dbt_project.yml`, with the track's display name and
  description next to them. That block is the only per-track config.
...
- **No `_fivetran_deleted` reference in any model.** Query-based Postgres
  replication may not create it, and a missing column would break every
  attendee at once. It is covered in `snowflake/GOTCHAS.md` instead.
...
### The dbt MCP Server cannot currently be registered directly in Snowflake AI
...
not exist at build time, and the only Snowflake account reachable from the
build environment does not hold the Fivetran-landed schemas. Parse is as far as
it goes without credentials.
...
Other things to confirm on the dry run:

1. **Identifier casing.** Whether the Fivetran Snowflake destination lands
   upper or lower case decides whether `quoting` in `dbt_project.yml` is right.
   Currently set to unquoted, which suits a standard destination. Covered as
   gotcha 3.
2. **Whether Fivetran creates `_fivetran_deleted`** on a query-based Postgres
   connector. No model depends on it, so this is informational.

### Placeholders left for the author

`{{FIVETRAN_WORKSHOP_URL}}`, `{{DBT_WORKSHOP_URL}}`, `{{LAB_CREDENTIALS_URL}}`,
`{{PASSCODE}}`, `{{POSTGRES_PASSWORD}}`, `{{SNOWFLAKE_ACCOUNT}}`,
`{{DBT_HOST}}`, `{{DBT_TOKEN}}`, `{{DBT_PROD_ENV_ID}}`, `{{DBT_USER_ID}}`.
All collected in one table in `docs/account-setup.md`.

The source Postgres host, port, database, user and update method are documented
inline in `fivetran/connector-setup.md` because attendees need to type them.
The password is a placeholder: this repo is forked publicly, and the lab
credentials card is the right channel for it.
